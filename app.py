...
# Function to pad single-digit ayah numbers
def fromtnum(n):
    newfromt = n
    if len(str(n)) == 1: newfromt = "00" + str(n)
    elif len(str(n)) == 2: newfromt = "0" + str(n)
    return newfromt
def download_and_send_surah(update, context):
    message_text = update.message.text
    try:
        surah_num = message_text
        surah_num = int(surah_num)
        
    except ValueError:
        context.bot.send_message(update.chat_id, "Invalid format! Please send surah_number ayah_number")
        return
    for ayah_num  in [1,surah_data[surah_num][1]]  
...
def  download_and_send_ayah(update, context):
    ...
    message_text = update.message.text
    try:
        surah_num, ayah_num = message_text.split(' ')
        surah_num = int(surah_num)
        ...
        return
    # Validate numbers
    if surah_num < 0 or surah_num >= len(surah_data):
        context.bot.send_message(update.chat_id, f"Invalid surah number: {surah_num}")
        return
    if ayah_num < 1 or ayah_num > surah_data[surah_num][1]:
        context.bot.send_message(update.chat_id, f"Invalid ayah number: {ayah_num}")
        return

    # Build download URL
    record = "Abdul_Basit_Mujawwad_128kbps/"  # Choose desired recitation
    url = f"http://verse.mp3quran.net/data/{record}/{fromtnum(surah_data[surah_num][0])}/{fromtnum(ayah_num)}.mp3"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            with open(SAVING_LOCATION + os.path.basename(url), 'wb') as f:
                f.write(response.content)

            # Send audio and additional information (optional)
            sent_audio = context.bot.send_audio(update.chat_id, open(SAVING_LOCATION + os.path.basename(url), 'rb'))
            os.remove(sent_audio.photo.file_id)  # Clean up downloaded file
            context.bot.send_message(update.chat_id, f"Surah {surah_data[surah_num][4]} - Ayah {ayah_num}")
        else:
            context.bot.send_message(update.chat_id, f"Error downloading ayah: {response.status_code}")
    except Exception as e:
        context.bot.send_message(update.chat_id, f"Error downloading ayah: {e}")

# Download and send ayah function
def download_and_send(surah_num, ayah_num):
